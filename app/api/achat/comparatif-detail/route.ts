...
export async function GET(request: NextRequest) {
  try {
    ...
    const cacheKey = `achat-detail:${m}:${fromN1}:${toN1}:${fromN2}:${toN2}`;

    const result = await cached(cacheKey, async () => {
      const [linesN1r, linesN2r, srcN1, srcN2] = await Promise.all([
        ...
        query<SourceRow>(ACHAT_VENTES_COMPARATIF_PAR_SOURCE, [fromN1, toN1, m]),
        query<SourceRow>(ACHAT_VENTES_COMPARATIF_PAR_SOURCE, [fromN2, toN2, m]),
      ]);
      ...
      return {
        n1: { lines: linesN1r.map(formatLine), parSource: n1ParSource },
        n2: { lines: linesN2r.map(formatLine), parSource: n2ParSource },
        evolutionParSource,
      };
    }, TTL.DEFAULT);
