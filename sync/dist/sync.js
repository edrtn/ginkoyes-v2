...
function restoreGbk(cfg, log) {
    ...
    try {
        ...
        const isTimeout = err instanceof Error && (err.killed === true ||
            err.signal === 'SIGTERM' ||
...
async function syncTable(cfg, pool, table, log) {
    ...
    try {
        (0, child_process_1.execSync)(cmd, { stdio: "pipe", timeout: 1800000 }); // 30 min max par table
    }
    catch (err) {
    ...
    try {
        ...
        const flushBatch = async () => {
            ...
            await conn.query(`INSERT IGNORE INTO ${table.name} (${table.columns.join(", ")}) VALUES ${multi}`, batch.flat());
            batch = [];
        };
...
async function runSync(cfg) {
    ...
    try {
        ...
        await pool.execute("UPDATE _sync_meta SET sync_end = NOW(), status = 'success', tables_synced = ?, rows_synced = ?, duration_ms = ? WHERE id = ?", [tablesSynced, totalRows, durationMs, syncId]);
        ...
        await pool.execute("UPDATE _sync_meta SET sync_end = NOW(), status = 'error', error_message = ?, duration_ms = ? WHERE id = ?", [errorMsg, durationMs, syncId]);
