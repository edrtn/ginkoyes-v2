...
SELECT
  ...
  cou.COU_NOM AS COULEUR,
  cmd.QTE_COMMANDEE AS QTE_RECUE,
  COALESCE((
    SELECT SUM(tkl.TKL_QTE)
    FROM CSHTICKETL tkl
    JOIN CSHTICKET t ON t.TKE_ID = tkl.TKL_TKEID
    WHERE tkl.TKL_ARTID = a.ART_ID AND tkl.TKL_COUID = cmd.COU_ID
      AND t.TKE_DATE >= COALESCE(rec.DATE_RECEPTION, '1900-01-01')
  ), 0) + COALESCE((
    SELECT SUM(bll.BLL_QTE)
    FROM NEGBLL bll
    JOIN NEGBL bl ON bl.BLE_ID = bll.BLL_BLEID
    WHERE bll.BLL_ARTID = a.ART_ID AND bll.BLL_COUID = cmd.COU_ID
      AND bl.BLE_DATE >= COALESCE(rec.DATE_RECEPTION, '1900-01-01')
  ), 0) AS QTE_VENDUE,
...
LEFT JOIN (
  SELECT rl.BRL_ARTID AS ART_ID, rl.BRL_COUID AS COU_ID,
    MIN(r.BRE_DATE) AS DATE_RECEPTION
  FROM recbrl rl
  JOIN recbr r ON r.BRE_ID = rl.BRL_BREID
  WHERE r.BRE_COLID = ?
  GROUP BY rl.BRL_ARTID, rl.BRL_COUID
) rec ON rec.ART_ID = cmd.ART_ID AND rec.COU_ID = cmd.COU_ID
